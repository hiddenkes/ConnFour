...
function App(options) {
	...
	//Set the player.
    this.player = this.settings.firstmove === 3 ? 1 + Math.round(Math.random()) : this.settings.firstmove;
    //Set the theme.
    this.theme = this.settings.theme;
    //Set the difficulty.
    this.difficulty = this.settings.difficulty;

    //Sets two player. This is done differently because it should be able to change more on-the-fly.
	...
	if (this.player === 2) {
	    ...
	    window.setTimeout(function () {
	        //We have to flip the player because doComputer does that automatically for some reason. We could fix this if we wanted, but just doing this requires the least code change.
	        that.flipPlayer();
	        that.doComputer();
	    }, 500);
	}
...
App.prototype = {
	...
	drawBoard: function(){
		...
		$("#backbutton").addClass(this.theme);
	...
	reset: function(){
		...
		$(".playerPiece").each(function(id, el){
			$(this).animate({
				top: "-1000px"
			}, Math.round(800 + Math.random() * 500), "easeInExpo");
		});
